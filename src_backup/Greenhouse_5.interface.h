...
void lcdPrintOutput(String item, byte _column, byte _row, Device device){
...
void lcdPrintOutputs(){
  ...
  if((greenhouse.devices.value() == 1)&&(greenhouse.heaters.value() == 0)){
    lcdPrintOutput("D1: ", 0, 3, D1);
  }
  else if((greenhouse.devices.value() >= 1)&&(greenhouse.heaters.value() >= 1)){
    lcdPrintOutput("D1: ", 0, 3, D1);
    lcdPrintOutput("|H1: ", 9, 3, H1);
  }
  else if((greenhouse.devices.value() == 2)&&(greenhouse.heaters.value() == 0)){
    lcdPrintOutput("D1: ", 0, 3, D1);
    lcdPrintOutput("|F2: ", 9, 3, D2);
  }
  else if((greenhouse.devices.value() == 0)&&(greenhouse.heaters.value() == 1)){
    lcdPrintOutput("H1: ", 0, 3, H1);
  }
  else if((greenhouse.devices.value() == 0)&&(greenhouse.heaters.value() == 2)){
    lcdPrintOutput("H1: ", 0, 3, H1);
  ...
  if(greenhouse.isActive(FIX_D1)){
      lcd.setCursor(8, 3);
      lcd.write((byte)1);
  }
  if(greenhouse.isActive(FIX_D2)){
      lcd.setCursor(19, 3);
      lcd.write((byte)1);
  }
  if(greenhouse.isActive(FIX_H1)&&greenhouse.devices.value() == 0){
      lcd.setCursor(8, 3);
      lcd.write((byte)1);
  }
  if(greenhouse.isActive(FIX_H1)&&greenhouse.devices.value() >= 1){
      lcd.setCursor(19, 3);
      lcd.write((byte)1);
  }
  ...
  if(greenhouse.isActive(DESHUM_AUTO1)&&greenhouse.devices.value() == 0){
      lcd.setCursor(8, 3);
      lcd.write((byte)2);
  }
  if(greenhouse.isActive(DESHUM_AUTO1)&&greenhouse.devices.value() >= 1){
      lcd.setCursor(19, 3);
      lcd.write((byte)2);
  }
  ...
  if(H1.isLock()&&greenhouse.devices.value() == 0){
      lcd.setCursor(8, 3);
      lcd.write((byte)4);
  }
  if(H1.isLock()&&greenhouse.devices.value() >= 1){
      lcd.setCursor(19, 3);
      lcd.write((byte)4);
  }
  if(H2.isLock()){
...
void deviceDisplay(Device device){

  float onTemp = greenhouse.coolingTemp() + device.mod.value();
  float offTemp = greenhouse.coolingTemp() + device.mod.value() - device.hyst.value();

  printHeader("DEVICE - PARAMETERS ",3);
  if(firstPrint == true){
    lcd.print("(");
    lcd.print(device.nb()+1);
    lcd.print(")");
    firstPrint = false;
  }
  adjustLine();
  ...
  for(int x = 1; x < 4; x++){
    ...
    switch(writeLine){
      ...
      case 3: lcd.setCursor(0,x);lcd.print(F("MOD:")); if(device.mod.value() >= 0){lcd.print(F(" "));}lcd.print(device.mod.value()); lcd.setCursor(9,3);lcd.print(F("|HYST: ")); lcd.print(device.hyst.value());break;
    }
  }
}
...
void deviceMenu(const __FlashStringHelper* header, Device &device){

  if(greenhouse.devices.value() >= device.nb()+1){
    char index = (device.nb()+1) +'0';

    char FHYST[Code_lenght]    = {'2',index,'0','0'};
    char FMOD[Code_lenght]   = {'2',index,'0','1'};

    if(programSelected(FHYST)){
      setParameter(header,F("HYSTERESIS"),device.hyst,FMOD);
    }
    if(programSelected(FMOD)){
      setParameter(header,F("MODIFICATOR"),device.mod,MENU1);
    }
  }
}
...
//Général
  ...
  if(programSelected(MAIN)){
    ...
    for(int x = 1; x <= greenhouse.devices.value();x++){
...
timepointMenu(F("NIGHT: "), T4);
  ...
  if(programSelected(D1OV)){
    confirmVariable(F(""),F("   F1 - OVERRIDE"), 0, "AUTO", "FORCE ON", "FORCE OFF");
    if(choiceIsConfirmed()){
      switch (typeSet) {
        case 0: D1.unlock();displayMenu('1');break;
        case 1: displayNextProgram(D1FOPEN);break;
        case 2: displayNextProgram(D1FCLOSE);break;
      }
    }
  }

  if(programSelected(D1FOPEN)){
    confirmVariable(F(""),F("  F1 - DELAY (MIN)"), (unsigned short)0, (unsigned short)0, (unsigned short)1440);
    zeroEqualInfinity();
    if(choiceIsConfirmed()){
      if(usvariable != 0){
        D1.lockOnAndWait(usvariable);
      }
      else{
        D1.lockOn();
      }
  ...
  if(programSelected(D1FCLOSE)){
    confirmVariable(F(""),F("  F1 - DELAY (MIN)"), (unsigned short)0, (unsigned short)0, (unsigned short)1440);
    zeroEqualInfinity();
    if(choiceIsConfirmed()){
      if(usvariable != 0){
        D1.lockOffAndWait(usvariable);
      }
      else{
        D1.lockOff();
      }
  ...
  if(programSelected(D2OV)){
    confirmVariable(F(""),F("   F2 - OVERRIDE"), 0, "AUTO", "FORCE ON", "FORCE OFF");
    if(choiceIsConfirmed()){
      switch (typeSet) {
        case 0: D2.unlock();displayMenu('1');break;
        case 1: displayNextProgram(D2FOPEN);break;
        case 2: displayNextProgram(D2FCLOSE);break;
      }
    }
  }

  if(programSelected(D2FOPEN)){
    confirmVariable(F(""),F("  F2 -  DELAY (MIN)"), (unsigned short)0, (unsigned short)0, (unsigned short)1440);
    zeroEqualInfinity();
    if(choiceIsConfirmed()){
      if(usvariable != 0){
        D2.lockOnAndWait(usvariable);
      }
      else{
        D2.lockOn();
      }
  ...
  if(programSelected(D2FCLOSE)){
    confirmVariable(F(""),F("  F2 -  DELAY (MIN)"), (unsigned short)0, (unsigned short)0, (unsigned short)1440);
    zeroEqualInfinity();
    if(choiceIsConfirmed()){
      if(usvariable != 0){
        D2.lockOffAndWait(usvariable);
      }
      else{
        D2.lockOff();
      }
