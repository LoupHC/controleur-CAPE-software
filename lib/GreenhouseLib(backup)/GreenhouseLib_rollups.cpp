...
Rollup::Rollup(){
  ...
  for(int x = 0; x < MAX_OVERRIDES;x++){
    overRide[x].active.setValue(false);
    overRide[x].target.setValue(0);
  }
...
void Rollup::initOverride(byte number, unsigned short targetIncrement){
  if((number > 0)&&(number < MAX_OVERRIDES)){
    overRide[number].target.setValue(targetIncrement);
  }
}

void Rollup::setOverride(byte number, boolean state){
  if((number > 0)&&(number < MAX_OVERRIDES)){
    if(state == true){
      if((overRide[number].active.value() == false) && (overRide[number].type == FLOATING)){
        overRide[number].counter = 0;
      }
      overRide[number].active.setValue(true);
    }
    else{
      overRide[number].active.setValue(false);
    }
  }
}
/*
void Rollup::createOverride(byte ID, byte priority, byte hrStart, byte mnStart, byte hrStop, byte mnStop, byte target){
  overRide[priority].ID = ID;
  overRide[priority].type = FIX;
  overRide[priority].hrStart.value() = hrStart;
  overRide[priority].mnStart.value() = mnStart;
  overRide[priority].hrStop.value() = hrStop;
  overRide[priority].mnStop.value() = mnStop;
  overRide[priority].target.value() = target;
}
void Rollup::createOverride(byte ID, byte priority, byte mn, byte target){
  overRide[priority].ID = ID;
  overRide[priority].type = FLOATING;
  overRide[priority].mnStop.value() = mn;
  overRide[priority].target.value() = target;
}

void Rollup::createOverride(byte ID, byte priority, byte comparator, float variable, byte target){
  overRide[priority].ID = ID;
  overRide[priority].type = FLOATRELATIVE;
  overRide[priority].comparator.value() = comparator;
  overRide[priority].floatVar.value() = variable;
  overRide[priority].target.value() = target;
}

void Rollup::createOverride(byte ID, byte priority, bool variable, byte target){
  overRide[priority].ID = ID;
  overRide[priority].type = BOOLRELATIVE;
  overRide[priority].boolVar.value() = variable;
  overRide[priority].target.value() = target;
}


void Rollup::checkOverride(byte ID, byte hr, byte mn){
  for(int x = 0;x < MAX_OVERRIDES;x++){
    if(overRide[x].ID == ID){
      if(isBetween(overRide[x].hrStart.value(), overRide[x].mnStart.value(), hr, mn, overRide[x].hrStop.value(), overRide[x].mnStop.value())){
        setOverride(x, true);
      }
      else{
        setOverride(x, false);
...
void Rollup::checkOverride(byte ID){
  for(int x = 0;x < MAX_OVERRIDES;x++){
    if(overRide[x].ID == ID){
      if (overRide[x].counter >= (unsigned long)overRide[x].delay.value()*60000){
        setOverride(x, true);
      }
      else{
        setOverride(x, false);
...
void Rollup::checkOverride(byte ID, float variable){
  for(int x = 0;x < MAX_OVERRIDES;x++){
    if(overRide[x].ID == ID){
      if (overRide[x].comparator.value() == ABOVE){
        if (variable > overRide[x].floatVar){
          setOverride(x, true);
        }
        else{
          setOverride(x, false);
        ...
        if (overRide[x].comparator.value() == BELOW){
          if (variable < overRide[x].floatVar){
            setOverride(x, true);
          }
          else{
...
void Rollup::checkOverride(byte ID,bool variable){
  for(int x = 0;x < MAX_OVERRIDES;x++){
    if(overRide[x].ID == ID){
      if (variable == overRide[x].boolVar){
        setOverride(x, true);
      }
      else{
        setOverride(x, false);
...
void Rollup::lockOpen(){
  overRide[0].target.setValue(_increments);
  overRide[0].active.setValue(true);
}
void Rollup::lockClose(){
  overRide[0].target.setValue(0);
  overRide[0].active.setValue(true);
}
void Rollup::lockAtIncrement(byte increment){
  if(increment < _increments){
    overRide[0].target.setValue(increment);
    overRide[0].active.setValue(true);
  }
}
void Rollup::unlock(){
  //_activeOverride = OFF_VAL;
  overRide[0].active.setValue(false);
}
/*
...
byte Rollup::activeOverride(){
  for(int x = 0; x < MAX_OVERRIDES;x++){
    if(overRide[x].active.value() == true){
      return  x;
    }
  }
  return OFF_VAL;
...
void Rollup::updatePosition(byte activeOverride){
  ...
  _incrementMove = (short)overRide[activeOverride].target.value() - (short)_incrementCounter;
}
  ...
  if(debugTimer > 1000){
    if(_localCounter == 0 ){
        ...
        for(int x = 0; x<MAX_OVERRIDES;x++){
          ...
          Serial.print(x);
          Serial.print(" IS : ");
          Serial.println(overRide[x].active.value());
          Serial.print("TARGET : ");
          Serial.println(overRide[x].target.value());
        }
      #endif
