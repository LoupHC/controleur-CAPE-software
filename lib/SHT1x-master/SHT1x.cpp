...
readSensor();
  ...
  _linearHumidity = C1 + C2 * _rawHumidity + C3 * _rawHumidity * _rawHumidity;
  // Correct humidity value for current temperature
  _correctedHumidity = (_temperature - 25.0 ) * (T1 + T2 * _rawHumidity) + _linearHumidity;
...
void SHT1x::readSensor()
{
  ...
  if((!waitingForTemp)&&(!waitingForHumidity)){
    //ask for temp raw value and init counter
    sendCommandSHT(_gTempCmd, _dataPin, _clockPin);
    waitingForTemp = true;
    requestDelay = 0;
  }
  if((requestDelay > 1000)&&(waitingForTemp)){
    //extract temp raw value and ask for humidity value
    _rawTemp = getData16SHT(_dataPin, _clockPin);
  ...
  if((requestDelay > 1000)&&(waitingForHumidity)){
    //extract humididty raw value
    _rawHumidity = getData16SHT(_dataPin, _clockPin);
    skipCrcSHT(_dataPin, _clockPin);
    waitingForHumidity = false;
    //Serial.print("_rawHumidity");
    //Serial.println(_rawHumidity);
  }
}
