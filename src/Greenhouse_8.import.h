...
void defaultSettings(){
    greenhouse.initGreenhouse(TIMEZONE, LATITUDE, LONGITUDE, TIMEPOINTS, ROLLUPS, STAGES, FANS, HEATERS);
    greenhouse.startingTime(rightNow);
...
void loadParameters(){
    ...
    if(greenhouse.fans.value() == 1){
      F1.initOutput(MCP23008, ACT_HIGH, DEVICE1_PIN);
    }
    if(greenhouse.heaters.value() == 1){
    ...
    if(greenhouse.fans.value() == 2){
      F1.initOutput(MCP23008, ACT_HIGH, DEVICE1_PIN);
      F2.initOutput(MCP23008, ACT_HIGH, DEVICE2_PIN);
    }
    if(greenhouse.heaters.value() == 2){
